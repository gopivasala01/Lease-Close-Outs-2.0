	...
	//public static void main(String args[]) throws Exception
	{
	  ...
	  //Increased Rent Check
	    ...
	    try
	    {
		    ...
		    {
		    	PDFReader.adminFee = text.substring(text.indexOf(PDFAppConfig.ChicagoPFW_Format1.adminFee_prior2)+PDFAppConfig.ChicagoPFW_Format1.adminFee_prior2.length()).split(" ")[0].trim();
	    		if(PDFReader.adminFee.matches(".*[a-zA-Z]+.*"))
			    {
	  ...
	  //Resident Benefits Package 
	    if(text.contains(PDFAppConfig.ChicagoPFW_Format1.residentBenefitsPackageAddendumCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
	    {
	  ...
	  //RBP Opt - Out Addendum Check
	    ...
	    try
	    {
	    	...
	    	String[] earlyTerminationRaw = earlyTerminationSectionText.substring(earlyTerminationSectionText.indexOf(PDFAppConfig.ChicagoPFW_Format1.AB_earlyTerminationFee_Prior)+PDFAppConfig.ChicagoPFW_Format1.AB_earlyTerminationFee_Prior.length()).split(" ");
...
//					 System.out.println("Pet rent = "+PDFReader.petRent.trim());
	  ...
	  //Smart Home Agreement
	    try
	    {
	    	if(text.contains(PDFAppConfig.ChicagoPFW_Format1.smartHomeAgreementCheck))
	    	{
	    		PDFReader.smartHomeAgreementCheck = true;
	    		System.out.println("Smart Home Agreement is available");
	    		try
	    		{
	    			PDFReader.smartHomeAgreementFee=text.substring(text.indexOf(PDFAppConfig.ChicagoPFW_Format1.smartHomeAgreementFee_Prior)+PDFAppConfig.ChicagoPFW_Format1.smartHomeAgreementFee_Prior.length()).trim().split(" ")[0];
	    		}
	    		catch(Exception e)
	    		{
