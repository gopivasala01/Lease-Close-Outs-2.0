...
public class Alabama_Format2 
{
	...
	//public static void main(String[] args) throws Exception 
	{
	  ...
	  //Resident Benefits Package 
	    if(text.contains(PDFAppConfig.Alabama_Format2.residentBenefitsPackageAddendumCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
	    {
	    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
	    	 try
	 	    {
	 		    PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.AB1_residentBenefitsPackage_Prior)+PDFAppConfig.Alabama_Format2.AB1_residentBenefitsPackage_Prior.length()).split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
	    ...
	    {
		    if(text.contains(PDFAppConfig.Alabama_Format2.HVACFilterAddendumTextAvailabilityCheck)==true)
		    {
		    ...
		    {
		    	PDFReader.HVACFilterFlag =true;
		    	PDFReader.airFilterFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.HVACAirFilter_prior)+PDFAppConfig.Alabama_Format2.HVACAirFilter_prior.length()).split(" ")[0].trim();
	  ...
	  //Prepayment Charge
		...
		try
	    {
	    	String[] earlyTerminationRaw = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.earlyTermination_Prior)+PDFAppConfig.Alabama_Format2.earlyTermination_Prior.length()).split(" ");
	    ...
	    if(PDFReader.petFlag ==true)
	    {
	    	...
	    	try
	    	{
	    	OKC_PropertyWare.petSecurityDeposit = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.securityDeposit_Prior)+PDFAppConfig.Alabama_Format2.securityDeposit_Prior.length()).trim().split(" ")[0];//,text.indexOf(PDFAppConfig.Alabama_Format2.securityDeposit_After));
	    	...
	    	*/
	    	...
	    	try
	    	{
	    	typeSubString = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.typeWord_Prior)+PDFAppConfig.Alabama_Format2.typeWord_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format2.typeWord_After));
	    	}
	    	catch(Exception e)
	    	{
	    		try
	    		{
	    		typeSubString = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.typeWord_Prior)+PDFAppConfig.Alabama_Format2.typeWord_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format2.typeWord_After2));
	    		}
	    		catch(Exception e2)
	    		{
	    			try
	    			{
	    				typeSubString = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.typeWord_Prior2)+PDFAppConfig.Alabama_Format2.typeWord_Prior2.length(),text.indexOf(PDFAppConfig.Alabama_Format2.typeWord_After3));
	    			}
	    			catch(Exception e3)
	    			{
	    		    typeSubString = "";
		    ...
		    {
		    	String type = typeSubString.substring(RunnerClass.nthOccurrence(typeSubString, "Type:", i+1)+PDFAppConfig.Alabama_Format2.AB_pet1Type_Prior.length(),RunnerClass.nthOccurrence(typeSubString, "Breed:", i+1)).trim();
		    ...
		    try
		    {
		    	PDFReader.petOneTimeNonRefundableFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.petOneTimeNonRefundable_Prior)+PDFAppConfig.Alabama_Format2.petOneTimeNonRefundable_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format2.petOneTimeNonRefundable_After)).trim();
		    ...
		    catch(Exception e)
		    {
		    	try
		    	{
		    		PDFReader.petOneTimeNonRefundableFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.petOneTimeNonRefundable_Prior2)+PDFAppConfig.Alabama_Format2.petOneTimeNonRefundable_Prior2.length()).trim().split(",")[0];
	  ...
	  //Service Animal Addendum check
	    try
	    {
	    if(text.contains(PDFAppConfig.Alabama_Format2.serviceAnimalText))
	    {
	    	PDFReader.serviceAnimalFlag = true;
    		System.out.println("Service Animal Addendum is available");
    		
            String typeSubString = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.AB_serviceAnimal_typeWord_Prior)+PDFAppConfig.Alabama_Format2.AB_serviceAnimal_typeWord_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format2.AB_serviceAnimal_typeWord_After));
	  ...
	  //Concession Addendum
	    ...
	    try
	    {
	    	if(text.contains(PDFAppConfig.Alabama_Format2.concessionAddendumText))
	    	{
	  ...
	  //Smart Home Agreement
	    try
	    {
	    	if(text.contains(PDFAppConfig.Alabama_Format2.smartHomeAgreementCheck))
	    	{
	    		PDFReader.smartHomeAgreementCheck = true;
	    		System.out.println("Smart Home Agreement is available");
	    		try
	    		{
	    			PDFReader.smartHomeAgreementFee=text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.smartHomeAgreementFee_Prior)+PDFAppConfig.Alabama_Format2.smartHomeAgreementFee_Prior.length()).trim().split(" ")[0];
	    		}
	    		catch(Exception e)
	    		{
...
// Check if Option 1 is selected in RBP Lease Agreement
	...
	public static boolean lateFeeRule()
	{
		...
		try
		{
		 lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.lateFeeRuleText_Prior)+PDFAppConfig.Alabama_Format2.lateFeeRuleText_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format2.lateFeeRuleText_After));
		}
		catch(Exception e)
		{
			try
			{
			lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.lateFeeRuleText_Prior)+PDFAppConfig.Alabama_Format2.lateFeeRuleText_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format2.lateFeeRuleText_After2));
			}
			catch(Exception e2)
			{
			return false;
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format2.lateFeeRule_whicheverIsGreater))
		{
		...
		//Late charge day
		   ...
		   // PDFReader.lateChargeDay =  lateFeeRuleText.substring(lateFeeRuleText.indexOf(OKC_PDFAppConfig.lateFeeRule_whicheverIsGreater_dueDay_Prior)+OKC_PDFAppConfig.lateFeeRule_whicheverIsGreater_dueDay_Prior.length()).trim().split(" ")[0];
				PDFReader.lateChargeDay =  lateFeeRuleText.split(PDFAppConfig.Alabama_Format2.lateFeeRule_whicheverIsGreater_dueDay_After)[0].trim();
		...
		//Late Fee Percentage
			try
			{
		    PDFReader.lateFeePercentage =  lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Alabama_Format2.lateFeeRule_whicheverIsGreater_lateFeePercentage)+PDFAppConfig.Alabama_Format2.lateFeeRule_whicheverIsGreater_lateFeePercentage.length()).trim().split(" ")[0];
         ...
         try
         {
         String lateFeeAmount  = lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Alabama_Format2.lateFeeRule_whicheverIsGreater_lateFeeAmount)+PDFAppConfig.Alabama_Format2.lateFeeRule_whicheverIsGreater_lateFeeAmount.length()).trim().split(" ")[0];
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format2.lateFeeRule_mayNotExceedMoreThan30Days))
		{
			...
			PDFReader.lateFeeType ="initialFeePluPerDayFee"; 
	         try
	 	    {
	 		    PDFReader.lateChargeFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.AB_lateFee_Prior)+PDFAppConfig.Alabama_Format2.AB_lateFee_Prior.length()).trim().split(" ")[0];
	 	   ...
	 	   RunnerClass.initialFeeAmount = PDFReader.lateChargeFee;
	 	    ...
	 	    try
	 	    {
	 	    	PDFReader.lateFeeChargePerDay = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.AB_additionalLateChargesPerDay_Prior)+PDFAppConfig.Alabama_Format2.AB_additionalLateChargesPerDay_Prior.length()).split(" ")[0].trim();//,text.indexOf(OKC_PDFAppConfig.AB_additionalLateChargesPerDay_After));
	 	    }
	 	    catch(Exception e)
	 	    {
	 	    ...
	 	    try
	 	    {
	 	    	PDFReader.additionalLateChargesLimit = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.AB_additionalLateChargesLimit_Prior)+PDFAppConfig.Alabama_Format2.AB_additionalLateChargesLimit_Prior.length()).trim().split(" ")[0]; //,text.indexOf(OKC_PDFAppConfig.AB_additionalLateChargesLimit_After));
	 	    }
	 	    catch(Exception e)
	 	    {
	 	 ...
	 	 //Late Charge Day
		...
		else if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format2.lateFeeRule_mayNotExceedAmount)||lateFeeRuleText.contains("and late charges are paid in full"))
			{
			PDFReader.lateFeeRuleType = "initialFeePluPerDayFee";
			//PDFReader.lateFeeRuleType = "Initial Fee + Per Day Fee";
			...
			if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format2.lateFeeRule_totalDelinquentRentDueToTheTenantAccount))
			{
				PDFReader.lateFeeRuleType = "GreaterOfFlatFeeOrPercentage";
				RunnerClass.lateFeeType = "GreaterOfFlatFeeOrPercentage";
		...
		try
	    {
	    	PDFReader.lateChargeDay = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.lateFeeDay_Prior)+PDFAppConfig.Alabama_Format2.lateFeeDay_Prior.length()).trim().split(" ")[0].trim().replace("[^0-9]", "");
	    ...
	    try
	    {
	    	PDFReader.lateChargeFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.lateChargePerDayFee)+PDFAppConfig.Alabama_Format2.lateChargePerDayFee.length()).trim().split(" ")[0].trim();
	    	//PDFReader.lateChargeFee = PDFReader.lateChargeFee.replaceAll("[^.0-9]", "");
	    }
	    catch(Exception e)
	    {
	    ...
	    try
	    {
	    	PDFReader.additionalLateCharges = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.additionaLateCharge_Prior)+PDFAppConfig.Alabama_Format2.additionaLateCharge_Prior.length()).trim().split(" ")[0].trim();
	    	PDFReader.additionalLateCharges = PDFReader.additionalLateCharges.replaceAll("[^.0-9]", "");
	    }
	    catch(Exception e)
	    {
	    ...
	    {
	    	PDFReader.additionalLateChargesLimit = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.additionaLateCharge_Prior)+PDFAppConfig.Alabama_Format2.additionaLateCharge_Prior.length()).split(" ")[0].trim();
