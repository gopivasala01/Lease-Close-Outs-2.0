...
public class Alabama_Format1 
{
	...
	//public static void main(String args[]) throws Exception
	{
	  ...
	  //Monthly Rent Tax Check
	    try
	    {
	    	PDFReader.monthlyRentTaxAmount = text.substring(text.indexOf(PDFAppConfig.Alabama_Format2.monthlyRentTaxAmount)+PDFAppConfig.Alabama_Format2.monthlyRentTaxAmount.length()).split(" ")[0].trim().replace(",", "");
	    	
	    	if(RunnerClass.hasSpecialCharacters(PDFReader.monthlyRentTaxAmount.trim())==true||PDFReader.monthlyRentTaxAmount.trim().equalsIgnoreCase("0.00")||PDFReader.monthlyRentTaxAmount.trim().equalsIgnoreCase("N/A")||PDFReader.monthlyRentTaxAmount.trim().equalsIgnoreCase("n/a")||PDFReader.monthlyRentTaxAmount.trim().equalsIgnoreCase("na")||PDFReader.monthlyRentTaxAmount.trim().equalsIgnoreCase(""))
	    	{
	    		PDFReader.monthlyRentTaxAmount = "Error";
	    ...
	    try
	    {
		    PDFReader.adminFee = text.toLowerCase().substring(text.toLowerCase().indexOf(PDFAppConfig.Alabama_Format1.AB_adminFee_Prior.toLowerCase())+PDFAppConfig.Alabama_Format1.AB_adminFee_Prior.length()).trim().split(" ")[0];
	  ...
	  //Resident Benefits Package 
	    if(text.contains(PDFAppConfig.Alabama_Format1.residentBenefitsPackageAddendumCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
	    {
	    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
	    	 try
	 	    {
	 		    PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB1_residentBenefitsPackage_Prior)+PDFAppConfig.Alabama_Format1.AB1_residentBenefitsPackage_Prior.length()).split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
	    	...
	    	//PDFAppConfig.Alabama_Format1.AB1_residentBenefitsPackage_Prior
	    }
	    else
	    {
		    if(text.contains(PDFAppConfig.Alabama_Format1.HVACFilterAddendumTextAvailabilityCheck)==true)
		    {
		    ...
		    try
		    {
			   String[] airFilterFeeArray = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_airFilterFee_Prior)+PDFAppConfig.Alabama_Format1.AB_airFilterFee_Prior.length()).split(" ");
	  ...
	  //RBP Opt - Out Addendum Check
	    ...
	    try
	    {
	    	...
	    	String[] earlyTerminationRaw = earlyTerminationSectionText.substring(earlyTerminationSectionText.indexOf(PDFAppConfig.Alabama_Format1.AB_earlyTerminationFee_Prior)+PDFAppConfig.Alabama_Format1.AB_earlyTerminationFee_Prior.length()).split(" ");
	    	
		    PDFReader.earlyTermination = earlyTerminationRaw[0]+earlyTerminationRaw[1]; //text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_earlyTerminationFee_Prior)+PDFAppConfig.Alabama_Format1.AB_earlyTerminationFee_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_earlyTerminationFee_After));
	    }
	    catch(Exception e)
	    {
	    ...
	    try
	    {
		    ...
		    PDFReader.occupants = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_occupants_Prior)+PDFAppConfig.Alabama_Format1.AB_occupants_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_occupants_After));
	    }
	    catch(Exception e)
	    {
	    	try
	    	{
	    		 PDFReader.occupants = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_occupants_Prior2)+PDFAppConfig.Alabama_Format1.AB_occupants_Prior2.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_occupants_After2));
	    	}
	    	catch(Exception e2)
	    	{
	    		try
	    		{
	    			PDFReader.occupants = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_occupants_Prior3)+PDFAppConfig.Alabama_Format1.AB_occupants_Prior3.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_occupants_After3));
	    		}
	    		catch(Exception e3)
	    		{
	  ...
	  //Prepayment Charge
	    ...
	    petFlag = text.contains(PDFAppConfig.Alabama_Format1.AB_petAgreementAvailabilityCheck);
	    ...
	    if(petFlag ==true)
	    {
	    	...
	    	try
	    	{
	    	PDFReader.petSecurityDeposit = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_securityDeposity_Prior)+PDFAppConfig.Alabama_Format1.AB_securityDeposity_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_securityDeposity_After));
...
//					 System.out.println("Pet rent = "+PDFReader.petRent.trim());
		   ...
		   /* 
	    	try
    		{
    			//String petFeeRaw1 = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_petFee_Prior));
    			PDFReader.petFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_petFee_Prior)+PDFAppConfig.Alabama_Format1.AB_petFee_Prior.length()).trim().split(" ")[0].trim();
    			//PDFReader.petFee =  petFeeRaw[petFeeRaw.length-2].trim();
    			//if(PDFReader.petFee.matches(".*[a-zA-Z]+.*"))
    			//{
    				//PDFReader.petFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_petFee2_Prior)+PDFAppConfig.Alabama_Format1.AB_petFee2_Prior.length()).trim().split(" ")[0].trim();
    			//}
	    	...
	    	*/
	    	...
	    	String typeSubString = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_typeWord_Prior)+PDFAppConfig.Alabama_Format1.AB_typeWord_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_typeWord_After));
		    ...
		    {
		    	String type = typeSubString.substring(RunnerClass.nthOccurrence(typeSubString, "Type:", i+1)+PDFAppConfig.Alabama_Format1.AB_pet1Type_Prior.length(),RunnerClass.nthOccurrence(typeSubString, "Breed:", i+1)).trim();
		    ...
		    try
		    {
		    	PDFReader.petOneTimeNonRefundableFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_petFeeOneTime_Prior)+PDFAppConfig.Alabama_Format1.AB_petFeeOneTime_Prior.length()).split(" ")[0];//,text.indexOf(PDFAppConfig.Alabama_Format1.AB_petFeeOneTime_After));
	    ...
	    try
	    {
	    if(text.contains(PDFAppConfig.Alabama_Format1.serviceAnimalText))
	    {
	    	PDFReader.serviceAnimalFlag = true;
    		System.out.println("Service Animal Addendum is available");
    		
            String typeSubString = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_serviceAnimal_typeWord_Prior)+PDFAppConfig.Alabama_Format1.AB_serviceAnimal_typeWord_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_serviceAnimal_typeWord_After));
	    ...
	    try
	    {
	    	if(text.contains(PDFAppConfig.Alabama_Format1.concessionAddendumText))
	    	{
	  ...
	  //Smart Home Agreement
	    try
	    {
	    	if(text.contains(PDFAppConfig.Alabama_Format1.smartHomeAgreementCheck))
	    	{
	    		PDFReader.smartHomeAgreementCheck = true;
	    		System.out.println("Smart Home Agreement is available");
	    		try
	    		{
	    			PDFReader.smartHomeAgreementFee=text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.smartHomeAgreementFee_Prior)+PDFAppConfig.Alabama_Format1.smartHomeAgreementFee_Prior.length()).trim().split(" ")[0];
	    		}
	    		catch(Exception e)
	    		{
...
// Check if Option 1 is selected in RBP Lease Agreement
	...
	public static boolean lateFeeRule()
	{
		...
		try
		{
		 lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRuleText_Prior)+PDFAppConfig.Alabama_Format1.lateFeeRuleText_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRuleText_After));
		}
		catch(Exception e)
		{
			try
			{
			lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRuleText_Prior)+PDFAppConfig.Alabama_Format1.lateFeeRuleText_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRuleText_After2));
			}
			catch(Exception e2)
			{
				try
				{
				lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRuleText_Prior3)+PDFAppConfig.Alabama_Format1.lateFeeRuleText_Prior3.length(),text.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRuleText_After3));
				}
				catch(Exception e3)
				{
				return false;
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater))
		{
		...
		//Late charge day
			try
			{
		   // PDFReader.lateChargeDay =  lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater_dueDay_Prior)+PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater_dueDay_Prior.length()).trim().split(" ")[0];
				PDFReader.lateChargeDay =  lateFeeRuleText.split(PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater_dueDay_After)[0].trim();
		...
		//Late Fee Percentage
			try
			{
		    PDFReader.lateFeePercentage =  lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater_lateFeePercentage)+PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater_lateFeePercentage.length()).trim().split(" ")[0];
         ...
         try
         {
         String lateFeeAmount  = lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater_lateFeeAmount)+PDFAppConfig.Alabama_Format1.lateFeeRule_whicheverIsGreater_lateFeeAmount.length()).trim().split(" ")[0];
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format1.lateFeeRule_mayNotExceedMoreThan30Days))
		{
			...
			PDFReader.lateFeeType ="initialFeePluPerDayFee"; 
	         try
	 	    {
	 		    PDFReader.lateChargeFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_lateFee_Prior)+PDFAppConfig.Alabama_Format1.AB_lateFee_Prior.length()).trim().split(" ")[0];
	 	   ...
	 	   RunnerClass.initialFeeAmount = PDFReader.lateChargeFee;
	 	    ...
	 	    try
	 	    {
	 	    	PDFReader.lateFeeChargePerDay = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_additionalLateChargesPerDay_Prior)+PDFAppConfig.Alabama_Format1.AB_additionalLateChargesPerDay_Prior.length()).split(" ")[0].trim();//,text.indexOf(PDFAppConfig.Alabama_Format1.AB_additionalLateChargesPerDay_After));
	 	    }
	 	    catch(Exception e)
	 	    {
	 	    ...
	 	    try
	 	    {
	 	    	PDFReader.additionalLateChargesLimit = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_additionalLateChargesLimit_Prior)+PDFAppConfig.Alabama_Format1.AB_additionalLateChargesLimit_Prior.length()).trim().split(" ")[0]; //,text.indexOf(PDFAppConfig.Alabama_Format1.AB_additionalLateChargesLimit_After));
	 	    }
	 	    catch(Exception e)
	 	    {
	 	 ...
	 	 //Late Charge Day
		...
		else if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format1.lateFeeRule_mayNotExceedAmount)||lateFeeRuleText.contains(PDFAppConfig.Alabama_Format1.lateFeeRule_mayNotExceed375))
			{
			PDFReader.lateFeeRuleType = "initialFeePluPerDayFee";
			//RunnerClass.lateFeeRuleType = "Initial Fee + Per Day Fee";
			...
			if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format1.lateFeeRule_totalDelinquentRentDueToTheTenantAccount))
			{
				PDFReader.lateFeeRuleType = "GreaterOfFlatFeeOrPercentage";
				RunnerClass.lateFeeType = "GreaterOfFlatFeeOrPercentage";
		   ...
		   */
			...
			else if(lateFeeRuleText.contains(PDFAppConfig.Alabama_Format1.lateFeeRule_landlordTheLiquidatedSumOf))
			{
			PDFReader.lateFeeRuleType = "initialFeePluPerDayFee";
			//RunnerClass.lateFeeRuleType = "Initial Fee + Per Day Fee";
