	...
	//public static void main(String args[]) throws Exception
	{
	  ...
	  //Increased Rent Check
	    ...
	    try
	    {
		    ...
		    {
		    	PDFReader.adminFee = text.substring(text.indexOf(PDFAppConfig.Austin_Format1.adminFee_prior2)+PDFAppConfig.Austin_Format1.adminFee_prior2.length()).split(" ")[0].trim();
	    		if(PDFReader.adminFee.matches(".*[a-zA-Z]+.*"))
			    {
	  ...
	  //Resident Benefits Package 
	    ...
	    try
	    {
	    	...
	    	String[] earlyTerminationRaw = earlyTerminationSectionText.substring(earlyTerminationSectionText.indexOf(PDFAppConfig.Austin_Format1.AB_earlyTerminationFee_Prior)+PDFAppConfig.Austin_Format1.AB_earlyTerminationFee_Prior.length()).split(" ");
...
//					 System.out.println("Pet rent = "+PDFReader.petRent.trim());
	    ...
	    try
	    {
	    	if(text.contains(PDFAppConfig.Austin_Format1.smartHomeAgreementCheck))
	    	{
	    		PDFReader.smartHomeAgreementCheck = true;
	    		System.out.println("Smart Home Agreement is available");
	    		try
	    		{
	    			PDFReader.smartHomeAgreementFee=text.substring(text.indexOf(PDFAppConfig.Austin_Format1.smartHomeAgreementFee_Prior)+PDFAppConfig.Austin_Format1.smartHomeAgreementFee_Prior.length()).trim().split(" ")[0];
	    		}
	    		catch(Exception e)
	    		{
	...
	public static boolean lateFeeRule()
	{
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Austin_Format1.lateFeeRule_mayNotExceedMoreThan30Days))
		{
		   ...
		   */
			...
			else if(lateFeeRuleText.contains(PDFAppConfig.Austin_Format1.lateFeeRule_landlordTheLiquidatedSumOf))
			{
			PDFReader.lateFeeRuleType = "initialFeePluPerDayFee";
			//RunnerClass.lateFeeRuleType = "Initial Fee + Per Day Fee";
