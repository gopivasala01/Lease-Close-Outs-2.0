...
public class Arizona_Format1 
{
	...
	//public static void main(String args[]) throws Exception
	{
	    ...
	    try
	    {
	    	PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_commencementDate_Prior)+PDFAppConfig.Arizona_Format1.AB_commencementDate_Prior.length());
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_commencementDate_Prior2)+PDFAppConfig.Arizona_Format1.AB_commencementDate_Prior2.length());
	    		PDFReader.commencementDate = PDFReader.commencementDate.substring(0, PDFReader.commencementDate.indexOf(" (")).trim();
	    		PDFReader.commencementDate = PDFReader.commencementDate.trim().replaceAll(" +", " ");
	    	}
	    	catch(Exception e2)
	   ...
	   try
	    {
		   String expirationDateWaw = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_expirationDate_Prior)+PDFAppConfig.Arizona_Format1.AB_expirationDate_Prior.length());
		   PDFReader.expirationDate =expirationDateWaw.substring(0,expirationDateWaw.indexOf(PDFAppConfig.Arizona_Format1.AB_expirationDate_After)).trim();
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		 String expirationDateWaw = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_expirationDate_Prior2)+PDFAppConfig.Arizona_Format1.AB_expirationDate_Prior2.length());
	   ...
	   try
	    {
		    PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_proratedRent_Prior,1)+PDFAppConfig.Arizona_Format1.AB_proratedRent_Prior.length()).trim().split("as prorated rent")[0];
	   ...
	   System.out.println("Prorated Rent = "+PDFReader.proratedRent);
	    try
	    {
		    PDFReader.proratedRentDate = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_proratedRentDate_Prior)+PDFAppConfig.Arizona_Format1.AB_proratedRentDate_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_proratedRentDate_After)).trim();
	    }
	    catch(Exception e)
	    {
	    ...
	    try
	    {
		    PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_fullRentDate_Prior)+PDFAppConfig.Arizona_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_fullRentDate_After));
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_fullRentDate_Prior)+PDFAppConfig.Arizona_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_fullRentDate1_After));
			   	System.out.println("Monthly Rent Date= "+PDFReader.monthlyRentDate.trim());
	    	}
	    	catch(Exception e1)
		    {
	    ...
	    try
	    {
		    PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_fullRent_Prior)+PDFAppConfig.Arizona_Format1.AB_fullRent_Prior.length()).trim().split(" ")[0].trim();//,text.indexOf(PDFAppConfig.Arizona_Format1.AB_fullRent_After)).substring(1).replaceAll("[^.0-9]", "");;
		    if(RunnerClass.onlyDigits(PDFReader.monthlyRent.replace(".", "").replace(",", ""))==false||PDFReader.monthlyRent.replace(".", "").trim().length()==1)
		    {
		    	PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_fullRent2_Prior)+PDFAppConfig.Arizona_Format1.AB_fullRent2_Prior.length()).trim().split(" ")[0].trim();
		    }
	  ...
	  //Monthly Rent Tax Check
	    try
	    {
	    	...
	    	else
	    	{
	    		...
	    		try
	    		{
	    		PDFReader.totalMonthlyRentWithTax = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.totalMonthlyRent)+PDFAppConfig.Arizona_Format1.totalMonthlyRent.length()).split(" ")[0].trim();
	    		if(PDFReader.totalMonthlyRentWithTax.matches(".*[a-zA-Z]+.*")||PDFReader.totalMonthlyRentWithTax.replace(".", "").trim().length()==1)
			    {
	    			PDFReader.totalMonthlyRentWithTax = text.substring(text.indexOf(PDFAppConfig.Arizona_Format2.totalMonthlyRent2,1)+PDFAppConfig.Arizona_Format2.totalMonthlyRent2.length()).split(" ")[0].trim();
			    }
	    		}
	    		catch(Exception e)
	    		{
	  ...
	  //Increased Rent Check
	    try
	    {
	    	if(PDFReader.monthlyRent.contains("*")||text.contains(PDFAppConfig.Arizona_Format2.monthlyRentAvailabilityCheck)==true)
	    	{
	    		...
	    		//if(endDateArray[2].trim().length()==4)//&&RunnerClass.onlyDigits(endDateArray[2]))
	    		 //{
	    		  ...
	    		  String newRentStartDate[] = text.substring(text.indexOf(PDFAppConfig.Arizona_Format2.increasedRent_newStartDate_Prior)+PDFAppConfig.Arizona_Format2.increasedRent_newStartDate_Prior.length()).trim().split(" ");
	    		  ...
	    		  String increasedRentRaw = text.substring(text.indexOf(PDFAppConfig.Arizona_Format2.increasedRent_newStartDate_Prior)+PDFAppConfig.Arizona_Format2.increasedRent_newStartDate_Prior.length()).trim();
	    ...
	    try
	    {
		    PDFReader.adminFee = text.toLowerCase().substring(text.toLowerCase().indexOf(PDFAppConfig.Arizona_Format1.AB_adminFee_Prior.toLowerCase())+PDFAppConfig.Arizona_Format1.AB_adminFee_Prior.length()).trim().split(" ")[0];
		    if(PDFReader.adminFee.matches(".*[a-zA-Z]+.*"))
		    {
		    	PDFReader.adminFee = text.toLowerCase().substring(text.toLowerCase().indexOf(PDFAppConfig.Arizona_Format1.AB_adminFee_Prior2.toLowerCase())+PDFAppConfig.Arizona_Format1.AB_adminFee_Prior2.length()).trim().split(" ")[0];
		    	if(PDFReader.adminFee.matches(".*[a-zA-Z]+.*"))
			    {
		    		PDFReader.adminFee = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.adminFee_prior2)+PDFAppConfig.Arizona_Format1.adminFee_prior2.length()).split(" ")[0].trim();
		    		if(PDFReader.adminFee.matches(".*[a-zA-Z]+.*"))
				    {
	  ...
	  //Resident Benefits Package 
	    if(text.contains(PDFAppConfig.Arizona_Format1.residentBenefitsPackageAddendumCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
	    {
	    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
	    	 try
	 	    {
	 		    PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB1_residentBenefitsPackage_Prior)+PDFAppConfig.Arizona_Format1.AB1_residentBenefitsPackage_Prior.length()).split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
	    	...
	    	//PDFAppConfig.Arizona_Format1.AB1_residentBenefitsPackage_Prior
	    }
	    else
	    {
		    if(text.contains(PDFAppConfig.Arizona_Format1.HVACFilterAddendumTextAvailabilityCheck)==true)
		    {
		    	PDFReader.HVACFilterFlag =true;
		    try
		    {
			   String[] airFilterFeeArray = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_airFilterFee_Prior)+PDFAppConfig.Arizona_Format1.AB_airFilterFee_Prior.length()).trim().split(" ");
	  ...
	  //RBP Opt - Out Addendum Check
	    ...
	    try
	    {
		    ...
		    PDFReader.occupants = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_occupants_Prior4)+PDFAppConfig.Arizona_Format1.AB_occupants_Prior4.length());//,text.indexOf(PDFAppConfig.Arizona_Format1.AB_occupants_After4));
		    PDFReader.occupants = PDFReader.occupants.substring(0,PDFReader.occupants.indexOf("11"));
	    }
	    catch(Exception e)
	    {
	    	try
	    	{
	    		 PDFReader.occupants = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_occupants_Prior2)+PDFAppConfig.Arizona_Format1.AB_occupants_Prior2.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_occupants_After2));
	    	}
	    	catch(Exception e2)
	    	{
	    		try
	    		{
	    			PDFReader.occupants = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_occupants_Prior3)+PDFAppConfig.Arizona_Format1.AB_occupants_Prior3.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_occupants_After3));
	    		}
	    		catch(Exception e3)
	    		{
	  ...
	  //Prepayment Charge
	    ...
	    petFlag = text.contains(PDFAppConfig.Arizona_Format1.AB_petAgreementAvailabilityCheck);
	    ...
	    if(petFlag ==true)
	    {
	    	...
	    	try
	    	{
	    	PDFReader.petSecurityDeposit = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_securityDeposity_Prior)+PDFAppConfig.Arizona_Format1.AB_securityDeposity_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_securityDeposity_After));
	    	...
	    	try
		    {
	    		 PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_petRent_Prior)+PDFAppConfig.Arizona_Format1.AB_petRent_Prior.length()).trim().split(" ")[0];
	    		 ...
	    		 else
	    		 {
		    		 if(PDFReader.petRent.matches(".*[a-zA-Z]+.*")==true)
		    			 PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_petRent1_Prior)+PDFAppConfig.Arizona_Format1.AB_petRent1_Prior.length()).trim().split(" ")[0];
		    		 else 
...
//					 System.out.println("Pet rent = "+PDFReader.petRent.trim());
	    	...
	    	//Pet Rent with Taxes
	    	 try
	 	    {
	 	    	PDFReader.petRentTaxAmount = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.petRentTaxAmount_Prior)+PDFAppConfig.Arizona_Format1.petRentTaxAmount_Prior.length()).split(" ")[0].trim().replace(",", "");
		   ...
		   /* 
	    	try
    		{
    			//String petFeeRaw1 = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_petFee_Prior));
    			PDFReader.petFee = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_petFee_Prior)+PDFAppConfig.Arizona_Format1.AB_petFee_Prior.length()).trim().split(" ")[0].trim();
    			//PDFReader.petFee =  petFeeRaw[petFeeRaw.length-2].trim();
    			//if(PDFReader.petFee.matches(".*[a-zA-Z]+.*"))
    			//{
    				//PDFReader.petFee = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_petFee2_Prior)+PDFAppConfig.Arizona_Format1.AB_petFee2_Prior.length()).trim().split(" ")[0].trim();
    			//}
	    	...
	    	*/
	    	...
	    	String typeSubString = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_typeWord_Prior)+PDFAppConfig.Arizona_Format1.AB_typeWord_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_typeWord_After));
		    ...
		    {
		    	String type = typeSubString.substring(RunnerClass.nthOccurrence(typeSubString, "Type:", i+1)+PDFAppConfig.Arizona_Format1.AB_pet1Type_Prior.length(),RunnerClass.nthOccurrence(typeSubString, "Breed:", i+1)).trim();
		    ...
		    try
		    {
		    	PDFReader.petOneTimeNonRefundableFee = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_petFeeOneTime_Prior)+PDFAppConfig.Arizona_Format1.AB_petFeeOneTime_Prior.length()).split(" ")[0];//,text.indexOf(PDFAppConfig.Arizona_Format1.AB_petFeeOneTime_After));
	    ...
	    try
	    {
	    if(text.contains(PDFAppConfig.Arizona_Format1.serviceAnimalText))
	    {
	    	PDFReader.serviceAnimalFlag = true;
    		System.out.println("Service Animal Addendum is available");
    		
            String typeSubString = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_serviceAnimal_typeWord_Prior)+PDFAppConfig.Arizona_Format1.AB_serviceAnimal_typeWord_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.AB_serviceAnimal_typeWord_After));
	    ...
	    try
	    {
	    	if(text.contains(PDFAppConfig.Arizona_Format1.concessionAddendumText))
	    	{
	  ...
	  //Smart Home Agreement
	    try
	    {
	    	if(text.contains(PDFAppConfig.Arizona_Format1.smartHomeAgreementCheck))
	    	{
	    		PDFReader.smartHomeAgreementCheck = true;
	    		System.out.println("Smart Home Agreement is available");
	    		try
	    		{
	    			PDFReader.smartHomeAgreementFee=text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.smartHomeAgreementFee_Prior)+PDFAppConfig.Arizona_Format1.smartHomeAgreementFee_Prior.length()).trim().split(" ")[0];
	    		}
	    		catch(Exception e)
	    		{
...
// Check if Option 1 is selected in RBP Lease Agreement
	...
	public static boolean lateFeeRule()
	{
		...
		try
		{
		 lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRuleText_Prior)+PDFAppConfig.Arizona_Format1.lateFeeRuleText_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRuleText_After));
		}
		catch(Exception e)
		{
			try
			{
			lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRuleText_Prior)+PDFAppConfig.Arizona_Format1.lateFeeRuleText_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRuleText_After2));
			}
			catch(Exception e2)
			{
				try
				{
				lateFeeRuleText = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRuleText_Prior3)+PDFAppConfig.Arizona_Format1.lateFeeRuleText_Prior3.length(),text.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRuleText_After3));
				}
				catch(Exception e3)
				{
				return false;
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater))
		{
		...
		//Late charge day
			try
			{
		   // PDFReader.lateChargeDay =  lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater_dueDay_Prior)+PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater_dueDay_Prior.length()).trim().split(" ")[0];
				PDFReader.lateChargeDay =  lateFeeRuleText.split(PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater_dueDay_After)[0].trim();
		...
		//Late Fee Percentage
			try
			{
		    PDFReader.lateFeePercentage =  lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater_lateFeePercentage)+PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater_lateFeePercentage.length()).trim().split(" ")[0];
         ...
         try
         {
         String lateFeeAmount  = lateFeeRuleText.substring(lateFeeRuleText.indexOf(PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater_lateFeeAmount)+PDFAppConfig.Arizona_Format1.lateFeeRule_whicheverIsGreater_lateFeeAmount.length()).trim().split(" ")[0];
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_mayNotExceedMoreThan30Days))
		{
			...
			PDFReader.lateFeeType ="initialFeePluPerDayFee"; 
	         try
	 	    {
	 		    PDFReader.lateChargeFee = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_lateFee_Prior)+PDFAppConfig.Arizona_Format1.AB_lateFee_Prior.length()).trim().split(" ")[0];
	 	   ...
	 	   RunnerClass.initialFeeAmount = PDFReader.lateChargeFee;
	 	    ...
	 	    try
	 	    {
	 	    	PDFReader.lateFeeChargePerDay = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_additionalLateChargesPerDay_Prior)+PDFAppConfig.Arizona_Format1.AB_additionalLateChargesPerDay_Prior.length()).split(" ")[0].trim();//,text.indexOf(PDFAppConfig.Arizona_Format1.AB_additionalLateChargesPerDay_After));
	 	    }
	 	    catch(Exception e)
	 	    {
	 	    ...
	 	    try
	 	    {
	 	    	PDFReader.additionalLateChargesLimit = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.AB_additionalLateChargesLimit_Prior)+PDFAppConfig.Arizona_Format1.AB_additionalLateChargesLimit_Prior.length()).trim().split(" ")[0]; //,text.indexOf(PDFAppConfig.Arizona_Format1.AB_additionalLateChargesLimit_After));
	 	    }
	 	    catch(Exception e)
	 	    {
	 	 ...
	 	 //Late Charge Day
		...
		else if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_mayNotExceedAmount)||lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_mayNotExceed375))
			{
			PDFReader.lateFeeRuleType = "initialFeePluPerDayFee";
			//RunnerClass.lateFeeRuleType = "Initial Fee + Per Day Fee";
			...
			if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_totalDelinquentRentDueToTheTenantAccount))
			{
				PDFReader.lateFeeRuleType = "GreaterOfFlatFeeOrPercentage";
				RunnerClass.lateFeeType = "GreaterOfFlatFeeOrPercentage";
		   ...
		   */
			...
			else if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_landlordTheLiquidatedSumOf))
			{
			PDFReader.lateFeeRuleType = "initialFeePluPerDayFee";
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_TenantshallBeAssessedALateFeeInTheAmount))
		{
			PDFReader.lateFeeRuleType = "GreaterOfFlatFeeOrPercentage";
			RunnerClass.lateFeeType = "GreaterOfFlatFeeOrPercentage";
	   ...
	   */
		...
		else if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_designatedPlaceOfPayment)&&lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.AB_lateFee_Prior))
		{
			PDFReader.lateFeeRuleType = "initialFeePluPerDayFee";
			//RunnerClass.lateFeeRuleType = "Initial Fee + Per Day Fee";
		...
		if(lateFeeRuleText.contains(PDFAppConfig.Arizona_Format1.lateFeeRule_designatedPlaceOfPayment)&&lateFeeRuleText.contains("a late charge equal to"))
		{
			PDFReader.lateFeeRuleType = "GreaterOfFlatFeeOrPercentage";
			RunnerClass.lateFeeType = "GreaterOfFlatFeeOrPercentage";
	   ...
	   */
			...
			if(lateFeeRuleText.contains("Any rent payment that is not actually received"))
			{
				PDFReader.lateFeeRuleType = "GreaterOfFlatFeeOrPercentage";
				RunnerClass.lateFeeType = "GreaterOfFlatFeeOrPercentage";
			...
			try
	 	    {
			PDFReader.lateChargeDay = lateFeeRuleText.substring(lateFeeRuleText.indexOf("a late charge on the ")+"a late charge on the ".length()).trim().split(" ")[0];
			PDFReader.lateChargeDay = PDFReader.lateChargeDay.replaceAll("[^0-9]", "");
	 	    }
			catch(Exception e)
	 	    {
